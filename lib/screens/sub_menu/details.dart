...
class Details extends StatefulWidget {
  ...
  Details({@required this.menuColor, @required this.menuID});
...
class _DetailsState extends State<Details> {
  ...
  @override
  void initState(){
    detailMenu = Provider.of<MenuProvider>(context, listen: false).menu;
    if(detailMenu == null){
      final List<MenuModel> detailTitle = Provider.of<MenuProvider>(context, listen: false).subMenuItems.toList();
      // detailMenu = detailTitle.first;
      detailMenu = detailTitle.firstWhere((submenu) => submenu.id == widget.menuID);
    }
    super.initState();
  }
  ...
  Widget _buildBottomButton(BuildContext context){
    return Positioned(
      ...
      right: 0,
      child: FlatButton(
        shape: ContinuousRectangleBorder(side: BorderSide(color: BasicBoxColor)),
        color: BasicBoxColor,
        child: Padding(
  ...
  void _buildSubmitForm(BuildContext context){
    ...
    Provider.of<MenuProvider>(context).addDetailMenu(detailMenu);
